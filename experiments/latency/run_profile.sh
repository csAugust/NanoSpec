...
python3 experiments/latency/profile_latency.py $COMMON_ARGS \
...
echo "=== All profiling done. Results in results/extra/logs/profile/ ==="
echo ""
echo "Summary (tail -5 each log):"
for f in results/extra/logs/profile/*.log; do
    echo "--- $(basename $f) ---"
    tail -8 "$f"
    echo ""
done
